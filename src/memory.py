...
class ConversationMemory:
    ...
    async def summarize_messages(self, messages: List[BaseMessage]) -> str:
        ...
        summary_prompt = PromptTemplates.get_conversation_summary_prompt(message_contents)
    ...
    async def get_conversation_context(
        ...
        if len(messages) >= 4:
            ...
            summary = await self.summarize_messages(messages[:-1])  # Exclude current message
...
class ContextWindowManager:
    ...
    def __init__(self, max_tokens: int = 8000):
        ...
        """
        self.max_tokens = max_tokens
        self.memory = ConversationMemory(max_tokens=int(max_tokens * 0.7))  # 70% for context
    ...
    async def prepare_messages(
        ...
        non_system_count = sum(1 for m in final_messages if not isinstance(m, SystemMessage))
