...
async def agent(state: AgentState):
    ...
    if agent_config.enable_memory and len(messages) > 5:
        try:
            ...
            logger.info("[Agent] Context managed: %d -> %d messages", len(messages), len(prepared_messages))
        except Exception as e:  # pylint: disable=broad-except
            logger.warning("[Agent] Context management failed: %s, using original messages", e, exc_info=True)
    ...
    non_system_messages = [m for m in prepared_messages if not isinstance(m, SystemMessage)]
    ...
    msg_structure = [f"{type(m).__name__}(len={len(str(m.content))})" for m in prepared_messages]
    logger.debug("[Agent] Prepared messages structure: %s", msg_structure)
    
    # Invoke LLM with tools
    try:
        response = llm_with_tools.invoke(prepared_messages)
        logger.info("[Agent] LLM response received, has tool calls: %s", bool(response.tool_calls))
        ...
        for i, m in enumerate(prepared_messages):
            logger.error("Msg %d: %s Content: %s...", i, type(m).__name__, str(m.content)[:100])
...
def should_continue(state: AgentState) -> Literal["tools", END]:
    ...
    if hasattr(last_message, 'tool_calls') and last_message.tool_calls:
        logger.debug("[Router] Continuing to tools (%d calls)", len(last_message.tool_calls))
        return "tools"
